...
flowchart TD
    ...
    MANAGER --> LETTER
    LETTER --> RESERVE["Резерв под договор подготовлен"]
    RESERVE --> SENT["Демо-отправка записана"]
    SENT --> ORDERDB
...
stateDiagram-v2
    ...
    ready_to_send --> reserved: подготовить резерв под договор
    reserved --> sent: демо-отправка записана
```
...
| `reserved` | Резерв под договор подготовлен; ответ готов к отправке |
| `sent` | Отправка записана в журнал |
| `error` | Работа сохранена; доступна кнопка «Повторить эту карточку» |

### Правила резерва под договор

Карточка открывает резерв после готового письма и свежей проверки склада.
Кнопка записывает товар, объём и время отдельным событием, переводит карточку
в состояние `reserved` и открывает отправку. Новая версия склада снова ведёт
к пересчёту, поэтому письмо и резерв всегда опираются на один снимок данных.

Стенд готовит запись резерва для демонстрации полного цикла. Рабочая версия
передаст эту запись в учётную или складскую систему компании и получит её
внутренний номер.
...
| `PATCH /api/orders` | Записать вопросы, принять ответ, пересчитать, подтвердить вариант, подготовить резерв или записать демо-отправку |
...
его, подготовьте резерв под договор и запишите демо-отправку.
...
вопросы, ответ, пересчёт, выбор, резерв и отправку в её событиях. Сначала покажите
...
сохраняет карточку, вопросы, решение руководителя, резерв и отправку в общий
журнал.
...
- [ ] После подтверждения решения появляется шаг «Подготовить резерв под
      договор», затем открывается отправка.
