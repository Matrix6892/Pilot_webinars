...
Основной эпизод вебинара строится вокруг живого остатка. Ведущий меняет запас
...
выбранного исполнителя и отдельно модель проверки из
`OPENCODE_REVIEWER_MODEL`; без этой переменной используется GPT-5.6 Sol.
При остановке связи карточку пересчитывает сервер без моделей. После
увеличения остатка заказ переходит из маршрута руководителя в готовый ответ, а
история сохраняет прежнее решение и письмо.
...
| Компания | `koler-sales` получает `websearch` по трём точным триггерам; найденные ссылки сохраняются для ручной проверки, открытие страницы текущим профилем не подтверждается |
...
flowchart TD
    ...
    WORKER -. "при поисковом триггере" .-> SEARCH["websearch<br/>ссылки для ручной проверки"]
    ...
    ANSWER --> ORDERS
    ROUTE -- "Решает руководитель" --> MANAGER["Подтверждение одного особого варианта"]
    MANAGER --> LETTER
    LETTER --> RESERVE["Запись резерва в журнале"]
...
| Программа | Подставляет цены и остатки, считает сумму, проверяет полномочия |
...
`koler-sales` разрешает ей `websearch`; текущий профиль не подтверждает
открытие найденной страницы. Сервер, D1 и программная проверка задают точки
чтения и записи.
...
3. профиль `koler-sales` в живом режиме выполняет публичный поиск по точным
   триггерам; найденные ссылки остаются материалом для ручной проверки.
...
5. программа подставляет товар, цену и остаток из каталога и снимка склада и
   проверяет маршрут;
...
| При живом OpenCode в журнале появляется событие поиска | Рабочая модель выполнила `websearch`; найденные ссылки сохранены для ручной проверки |
...
в этом режиме не выполняется: быстрый путь не анализирует фото моделью и не
запускает отдельную проверку.
...
7. программа включает профиль `koler-sales` с разрешением `websearch`, если
...
OpenCode получает свежий снимок склада, выбранный исполнитель готовит новое
письмо, а отдельная модель из `OPENCODE_REVIEWER_MODEL` проверяет обещания.
Кнопка для изменения из другого окна запускает тот же путь. При остановке
связи карточку завершает серверный расчёт по программным правилам.
...
сохранить свежее число, сервер возвращает текущий остаток и не перезаписывает
его устаревшим значением.
...
Программа выбирает профиль `koler-sales` с разрешением `websearch` по трём
условиям:
...
3. поле `website` пусто, компания указана конкретным названием, а тема или
   ...
   заказ, проверка, оборот, производство, парк, партнёрство или долгосрочный
   заказ — либо целый объём от 1 до 40 кг.

Программа обрезает пробелы, но не проверяет URL-валидность поля `website`.
Профиль запрещает остальные инструменты и разрешает только `websearch`.
Найденные HTTP(S)-ссылки сохраняются для ручной проверки, не более трёх.
Поиск по одному названию получает пометку «Совпадение по названию». Открытие
страниц текущим профилем не подтверждается, поэтому ссылки не считаются
проверенными фактами: программа оставляет `research.checked=false` и убирает
их из оснований решения. Заявки без этих триггеров используют профиль по
письму, каталогу, складу и правилам без внешнего поиска.
...
открывается действие записи отправки.
...
```text
карта пути
  ↓ роли и режимы названы
обычный заказ
  ↓ видны склад, цена и письмо
фото забора
  ↓ видны границы наблюдения
ответ клиента
  ↓ появляется расчёт
проверяемые основания
  ↓ факты связаны с источниками
изменение склада
  ↓ создаётся новая версия решения
пульт руководителя
  ↓ подтверждение сохраняется
ИНН и деловая гипотеза
  ↓ ссылки остаются для ручной проверки
общий журнал
...
### 3. Провести живой запуск моделей
...
### 4. Подготовить изменение склада
...
### 7. Показать роли модели и руководителя
...
### 8. Проверить поиск компании

Откройте заказ тракторного завода с ИНН. Покажите источники и гипотезу о
пробной партии. Укажите, что профиль разрешает `websearch`, сохраняет найденные
ссылки для ручной проверки и не подтверждает открытие страниц. Обсудите
следующий вопрос клиенту и возможный основной объём.

Реплика:

> Агент выполнил поиск. Ссылки требуют ручного открытия перед использованием
> как проверенных фактов.

### 9. Собрать журнал дня
...
### 10. Провести самостоятельную практику
...
### 11. Связать сценарий с процессом участника
...
6. Поиск по ИНН сохраняет ссылки для ручной проверки.
7. Журнал собирает события карточки и изменения склада по времени.
...
- [ ] При пустом `website` конкретное название компании включает
      ...
      1–40 кг.
- [ ] `koler-sales` разрешает `websearch`, сохраняет найденные ссылки для
      ручной проверки и не подтверждает открытие страниц.
...
- Профиль `koler-sales` разрешает `websearch`. Найденные ссылки сохраняются
  для ручной проверки; открытие страниц текущим профилем не подтверждается.
- Первичный запуск модели с разрешённым поиском получает 70 секунд. При
...
| Новый факт меняет старое решение | Остаток 40 кг создаёт маршрут руководителя, 180 кг после автоматического пересчёта даёт готовый ответ | Встроенный пульт запускает новый круг OpenCode; без связи карточку пересчитывает сервер | Выполнено |
...
| У системы есть «ручки и ножки» | Система читает D1 и фото из R2, выполняет `websearch`, пишет результат, события, пересчёты и состояния карточки | Найденные ссылки требуют ручного открытия; входящая и исходящая почта остаются следующим подключением | Выполнено в границах стенда |
| Команда видит общую историю | Карточка и журнал связывают заявку, переписку, действия, склад, решение, запись резерва и запись отправки; общая Google-таблица получает дневную выгрузку | Публичные действия резерва и отправки остаются записями D1; таблица получает данные в одну сторону | Выполнено для вебинара |
...
решения после обновления склада, передаёт особое условие руководителю и
собирает общую историю. Рабочая входящая и исходящая почта, управление сроком
хранения фото, обратная запись из учётных систем и рабочие роли доступа в
стенд не входят.
