...
Первостепенная задача стенда — за один связный показ доказать, что агентная
система ведёт входящую заявку до безопасного коммерческого действия: понимает
свободный текст, обращается к живым данным, просит недостающие сведения,
передаёт спорные обязательства руководителю и оставляет проверяемую историю.

«Колер» показывает полный путь входящей заявки:

```text
письмо или фото → понимание задачи → живые факты → деловой контекст
→ маршрут → действие человека → письмо → общая история
```
...
карточку по новой версии склада. Дефицитный заказ становится готовым ответом,
а история сохраняет прежнее решение и письмо.
...
- Общая таблица:
  [данные стенда «Колер»](https://docs.google.com/spreadsheets/d/1gabC2L8HOihMzPpp6pDeoMDtqXN9cEoGxwV-Al3PaYY/edit)

## Что видит участник

До отправки письма страница показывает:

- путь заказа от почты до общей истории;
- три маршрута: «Готов ответить», «Нужны детали», «Решает руководитель»;
...
| Демонстрационный расчёт | Cloudflare Worker | Быстрый маршрут, уточнения, расчёт забора и автоматический пересчёт склада |
...
| Подготовленные данные | Поставка сайта | Каталог, цены, правила, рынок, профили компаний и учебные примеры |
| Google Таблицы | Внешняя ссылка | Просмотр подготовленных товаров и правил, ручное открытие или импорт CSV |

Живым источником в D1 служит остаток и его версия. Каталог, цены, рыночный
срез и профили компаний поставляются с демонстрационными данными. Связанная
Google-таблица сейчас не питает расчёт сайта.
...
| Сильная модель | Проектирует инструкцию, проверяет риск, предлагает улучшения |
| Программа | Подставляет цены и остатки, считает сумму, охраняет полномочия |
| Руководитель | При дефиците, скидке или оплате после поставки выбирает один подготовленный вариант и открывает отправку письма |
...
вопросы или особые варианты. Руководитель согласует один особый вариант перед
отправкой.
...
6. сильная модель отмечает риск, а программная защита переводит спорный случай
   руководителю;
...
Права остаются явными. Остаток меняет ведущий, особый вариант согласует
...
| Пользовательское фото появляется в карточке, а в живом журнале видны осмотр или безопасный отказ | Система сохраняет вложение в R2 и выполняет отдельный ограниченный зрительный шаг |
...
| Скидка, дефицит или отсрочка останавливают отправку | Система соблюдает границу полномочий |
| Дефицит совпадает с оплатой после поставки | Агент учитывает оба условия и предлагает разделить поставку и оплату по партиям |
| Руководитель согласует один подготовленный вариант | Человек управляет обязательством компании |
...
уточнению, дефицит и особые условия ведут к руководителю. Загруженная
фотография сохраняется в R2 и карточке. Быстрый путь не анализирует
...
7. текстовая модель обращается к открытым источникам при деловой пользе;
8. программа повторно извлекает факты письма и подставляет каталожные числа;
9. сильная модель проверяет обещания и риск;
10. замечание рецензента с блокирующим риском переводит карточку руководителю;
11. результат и наблюдаемые события возвращаются в D1;
12. страница обновляет ту же карточку.
...
Ошибка загрузки показывается в форме; участник может выбрать другой файл или
продолжить без вложения. Сбой скачивания или зрительной модели не останавливает
уже созданный заказ. Журнал получает событие «Фото оставлено для уточнения», а
текстовая модель продолжает работу и задаёт клиенту вопросы. При успешном
проходе видны события «Модель зрения смотрит фото» и «Фото осмотрено».
...
расчёт. Встроенный пульт автоматически запускает быстрый серверный пересчёт
открытой карточки. Кнопка для изменения из другого окна запускает тот же путь.
Поэтому эффект 40 → 180 кг не зависит от доступности модели во время эфира.
После такого пересчёта карточка прямо подписывает результат как работу готовой
инструкции, живых данных и проверки программы.
...
умолчанию эту роль выполняет DeepSeek V4 Flash. Сильная модель возвращается в
двух местах: проверяет риск перед действием и изучает журнал после серии
заказов. Проход улучшения по журналу запускают вручную; стенд хранит готовую
инструкцию для этого шага.
...
очищает прежний выбор руководителя и заново определяет маршрут программными
правилами.

Встроенный пульт запускает серверный пересчёт при новой версии нужной краски.
...
решения. История использует уникальный ключ шага вида
`inventory:<круг>:<код краски>:<номер обновления>`.
...
Текст внутри фотографии также считается содержимым изображения, а не
инструкцией.

Быстрый режим сохраняет фото и определяет тему забора по тексту письма. Он
работает без зрительной модели. Ошибка MiMo в живом режиме приводит к тому же
безопасному ходу: вложение остаётся в карточке, а агент запрашивает сведения у
клиента.
...
- «Склад показывает объём для первой поставки»;
...
- [новость о производстве](https://www.fontanka.ru/2025/04/04/75300527/).

Заказ на 40 кг краски выглядит небольшим для крупного производителя. Агент
формулирует рабочую гипотезу о пробной партии и предлагает выяснить:

- что именно красят;
- как пройдёт испытание;
- когда клиент выберет основного поставщика;
- какой объём может последовать.

Факты и гипотеза показываются раздельно. Оценку расхода на один трактор
добавляем после появления подтверждённого источника.

В живом запуске рабочая модель может провести поиск по сайту, ИНН и открытым
публикациям. Руководитель получает ссылку, дату и короткий факт.
...
Руководитель отмечает один вариант переключателем и согласует его. Сервер
сохраняет вариант, время согласования и обновлённое письмо. После этого
открывается отправка.

Если дефицит совпал с оплатой после поставки, агент сохраняет оба условия в
основании решения. Один из трёх вариантов связывает поставки и оплату по
партиям, поэтому руководитель принимает цельное коммерческое решение.
...
stateDiagram-v2
    ...
    processing --> error: нужен новый запуск
    ...
    error --> recalc_result: восстановление карточки
...
Файл можно открыть или импортировать в Google Таблицы. Интерфейс предлагает
формулу
`=IMPORTDATA("https://koler-agent-demo.odinmaniac.chatgpt.site/api/ledger?format=csv")`
для первой ячейки листа. Перед эфиром ведущий практически проверяет доступ к
общей таблице и обновление данных по формуле. Остальные листы предназначены
для подготовленных товаров, правил, рынка и отраслевых примеров.
...
В зелёном наборе выберите «Повторный заказ краски для стен» и отправьте его.
...
Завершите отправку. Зритель получает короткий и понятный первый результат.

### 3. Получить первый момент узнавания через фотографию
...
При работающем OpenCode откройте события «Модель зрения смотрит фото» и «Фото
осмотрено». Покажите короткое наблюдение MiMo, затем вопросы текстового агента.
Отметьте, что фото помогает описать видимую сцену, а материал и размеры
подтверждает человек.

Отправьте вопросы клиенту. Вставьте подготовленный ответ с материалом и
размерами. Продолжите ту же карточку.
...
> В живом режиме MiMo описывает фото отдельным коротким проходом. Материал
> подтверждает клиент, а краску подбирает текстовый агент по каталогу. При
> недоступном зрительном вызове фото остаётся в карточке, и быстрый путь задаёт
> те же вопросы по тексту.
...
Карточка выберет маршрут руководителя и предложит способы закрыть дефицит.
Откройте основание: расчёт опирается на склад с 40 кг.
...
Карточка перечитает D1, сохранит новый снимок и программным расчётом перейдёт
в «Готов ответить». На экране останутся оба вывода и обе версии письма. Если
склад изменился из другого окна, нажмите «Обновить черновик по новому
остатку».
...
Подключите OpenCode и выберите DeepSeek V4 Flash. Возьмите заявку со скидкой,
отсрочкой или тендером. В журнале карточки покажите события «Выбран
исполнитель» и «Черновик передан сильной модели». Затем откройте маршрут
«Решает руководитель».
...
> точные числа, сильная модель проверяет риск, руководитель получает короткий
> выбор с последствиями и готовым текстом.

При отключённом OpenCode покажите тот же маршрут в быстром режиме и скажите,
что доказательство совместной работы двух моделей требует живого моста.
...
вопросы, ответ, пересчёт, выбор и отправку в её событиях. Затем откройте лист
«Заказы» общей Google Таблицы и проверьте обновление дневных данных по формуле
`IMPORTDATA`. CSV покажите как переносимую дневную выгрузку.
...
Уточните, что формула читает дневной CSV в одну сторону. Доступ к таблице и
фактическое обновление по формуле ведущий проверяет перед эфиром.

### 10. Перенести картину в компанию зрителя

Предложите участникам мысленно заменить:

- краски на свой каталог;
- D1 на свою учётную систему;
...
Стенд уже дал понятную карту. Зритель сам видит место для агентной системы в
своём процессе.

## Почему рассказ вызывает желание продолжить

Продажа строится через последовательное доказательство:
...
устройство, пользу и путь внедрения в своей компании.

## Чек-лист перед эфиром

- [ ] Публичный адрес открывается.
- [ ] Общая Google-таблица открывается по ссылке и обновляет дневные данные по
      указанной формуле `IMPORTDATA`.
...
- [ ] При живом OpenCode видны события «Модель зрения смотрит фото» и «Фото
      осмотрено».
...
- [ ] Дневной CSV скачивается и открывается в Google Таблицах.
- [ ] Ведущий разделяет живой журнал D1, ограниченную дневную выгрузку и
      внешнюю Google Таблицу с практически проверенным доступом.
...
- Изменение через встроенный пульт записывает новый факт и автоматически
  ...
  оставляет явную кнопку обновления. Карточка подписывает результат как работу
  готовой инструкции.
...
- Интерфейс показывает формулу
  `=IMPORTDATA("https://koler-agent-demo.odinmaniac.chatgpt.site/api/ledger?format=csv")`.
  Доступ к общей Google Таблице и обновление по формуле требуют практической
  проверки.
- CSV содержит текущий день и до 2000 строк каждого источника. Общий кеш
  ...
  источника.
- Подготовленный профиль компании обеспечивает устойчивый показ. Живой
  OpenCode расширяет его поиском по открытым источникам.
...
| Запрос без марки и веса | Готовое или пользовательское фото сохраняется; MiMo даёт короткое наблюдение; ответ клиента приводит к расчёту 30 кг | Зрительный вызов требует живого OpenCode; быстрый путь остаётся текстовым | Выполнено в обоих режимах, зрение — в живом |
...
| Новый факт меняет старое решение | Остаток 40 кг создаёт маршрут руководителя, 180 кг после автоматического пересчёта даёт готовый ответ | Встроенный пульт запускает программный путь | Выполнено |
| Недорогая модель следует сильной инструкции | DeepSeek V4 Flash получает открытую инструкцию, программа заземляет числа, GPT-5.6 Sol проверяет риск | Требуется активный локальный мост OpenCode | Выполнено в живом режиме |
| Спорное обязательство остаётся у руководителя | Агент готовит два-три варианта; при сочетании дефицита и оплаты после поставки учитывает оба условия; руководитель согласует один | Полномочия заданы демонстрационными правилами | Выполнено |
...
| Команда видит общую историю | Карточка и журнал связывают заявку, переписку, действия, склад, решение и демо-отправку; JSON и CSV дают ограниченные дневные выборки | Доступ и обновление Google Таблицы проверяют практически | Выполнено для вебинара |
...
смену решения вслед за живым складом, передачу риска человеку и общую
историю. Рабочая входящая и исходящая почта, управление сроком хранения фото,
