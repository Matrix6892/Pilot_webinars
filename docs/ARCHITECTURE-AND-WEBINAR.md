...
сценарий при остановке связи. Дефицитный заказ становится готовым ответом, а
...
2. Для подбора или расчёта не хватает факта — агент спрашивает только
   недостающее.
3. Дефицит, особая цена, оплата после поставки или жёсткий срок — агент
   приносит руководителю варианты с последствиями и готовым письмом.
...
7. программа выбирает источники: рутинная заявка и домен `.example` используют
   профиль продаж без публичного поиска; реальный сайт или ИНН разрешает
   поиск по открытым страницам;
8. публичный поиск получает 70 секунд; при долгом поиске программа один раз
   продолжает тот же заказ через профиль продаж по письму, каталогу, складу и
   правилам;
9. программа повторно извлекает факты письма и подставляет каталожные числа;
10. сильная модель проверяет обещания и риск;
11. замечание рецензента с блокирующим риском переводит карточку руководителю;
...
Ошибка сохраняет номер карточки, переписку, снимок склада, прежние решения и
историю. Интерфейс показывает кнопку «Повторить эту карточку». Кнопка ставит
ту же карточку в очередь; новый заказ и новый номер при таком продолжении
система не создаёт.
...
публикациям. Реальный сайт или ИНН открывает этот путь. Рутинная заявка и
домен `.example` используют профиль продаж по письму, каталогу, складу и
правилам. Руководитель получает ссылку, дату и короткий факт только после
успешного открытия страницы.

Поиск получает 70 секунд. При превышении этого времени программа записывает
событие «Продолжение по данным заказа» и один раз запускает профиль продаж без
внешнего поиска. Продолжение использует прежний номер карточки, переписку и
снимок склада.
...
- `research-fallback` → «Продолжение работы» и «Продолжено по данным заказа»;
...
- [ ] Рутинная заявка и домен `.example` проходят через профиль продаж без
      публичного поиска.
- [ ] Реальный сайт или ИНН разрешает поиск по открытым страницам.
- [ ] Долгий поиск один раз продолжает тот же заказ по письму, каталогу,
      складу и правилам.
- [ ] После ошибки кнопка «Повторить эту карточку» сохраняет номер,
...
- Рутинные заявки и домены `.example` проходят через профиль продаж без
  публичного поиска. Реальный сайт или ИНН разрешает поиск по открытым
  источникам.
- Публичный поиск получает 70 секунд. При долгом поиске система один раз
  продолжает ту же карточку через профиль продаж по письму, каталогу, складу
  и правилам.
- После ошибки кнопка «Повторить эту карточку» сохраняет номер, переписку,
