...
«Индустрию Покрытий»: в учебном предложении доступно 260 кг, для заказа нужно
200 кг. Ведущий уменьшает доступный объём партнёра до 150 кг. Агент запускает
...
ведущий увеличивает собственный остаток с 420 до 700 кг, и агент собирает весь
...
ведущим. Строка в таблице служит основанием для расчёта. Поставщик подтверждает
фактический объём и срок, технолог подтверждает цвет и совместимость
материалов. Стенд записывает подготовку резерва в журнал; фактический резерв
появляется после подключения системы поставщика.

При живой связи OpenCode снова запускает выбранного исполнителя и отдельно
модель проверки из `OPENCODE_REVIEWER_MODEL`; без этой переменной используется
GPT-5.6 Sol. При остановке связи карточку пересчитывает сервер без моделей.
...
Стенд считается готовым к вебинару, когда участник проходит наблюдаемый цикл
без терминала:
...
| Помощь при дефиците | Для заказа 620 кг агент сравнивает три учебных предложения, оставляет два с полным недостающим объёмом и выбирает поставщика по сроку, затем по цене. Карточка показывает 420 кг «Колера» по 415 ₽/кг, возможные 200 кг партнёра по 402 ₽/кг, расчёт 254 700 ₽ и выигрыш шести дней после подтверждений |
| Живое предложение партнёра | D1 хранит текущие учебные остатки партнёров. Переход 260 → 150 кг запускает новый круг, прежний вариант уступает следующему выполнимому предложению. Возврат 150 → 260 кг снова меняет решение |
...
| Цена | Карточка сопоставляет учебные цены поставщиков с нашей ценой и раскрывает основание вывода о марже |
...
| Роли | Инструкция MiMo задаёт правила наблюдения, программа ограничивает поля и размер; выбранная рабочая модель ведёт заказ; модель из `OPENCODE_REVIEWER_MODEL` отдельным запуском проверяет решение; программа сверяет числа |
...
flowchart TD
    ...
    PARTNERS[("D1 · учебные предложения партнёров<br/>остаток · цена · срок · дата")] --> SNAP
    ORDERS --> SNAP
    ...
    VISION --> WORKER["Выбранный исполнитель<br/>DeepSeek V4 Flash, DeepSeek V4 Pro<br/>или GPT-5.6 Sol"]
    ...
    WORKER --> GUARD
    GUARD --> REVIEW["Отдельная модель проверки<br/>OPENCODE_REVIEWER_MODEL<br/>по умолчанию GPT-5.6 Sol"]
...
| Drizzle и Cloudflare D1 | Облачная база стенда | Карточки, переписка, события, остатки своего склада, учебные предложения партнёров и решения руководителя |
...
| OpenCode | Компьютер ведущего | Запуск MiMo V2.5, выбранного исполнителя, разрешённого поиска и отдельной модели из `OPENCODE_REVIEWER_MODEL` |
| Подготовленные данные | Поставка сайта | Каталог, исходные цены и сроки партнёров, правила, профили компаний и учебные примеры |
...
В D1 находятся остатки своего склада и текущая версия учебных предложений
партнёров. Ключ `supplier-market-v1` в `system_state` хранит изменённый остаток
...
| Cloudflare D1 | Хранит заказы, переписку, склад, учебные предложения партнёров, историю и дневные показатели |
...
| Выбранный исполнитель | В живом режиме понимает смысл, выбирает источники, готовит следующий ход |
| Модель проверки | В живом режиме берётся из `OPENCODE_REVIEWER_MODEL` и отдельным запуском проверяет обещания, особые условия и границы полномочий; значение по умолчанию — GPT-5.6 Sol |
...
| Ведущий | Меняет учебный остаток своего склада или партнёра и показывает новый круг решения |
...
Каталог, склад и полномочия остаются отдельными проверяемыми данными при смене
рабочей модели.

## Где у агента «ручки и ножки»

Метафора относится ко всей агентной системе. Инструкция ограничивает
зрительную модель наблюдением, а рабочая модель понимает письмо. Профиль
...
2. сервер читает учебные предложения партнёров из D1 и добавляет их в тот же
   снимок;
3. связующий процесс может получить только готовое фото стенда или загруженное
...
4. выбранный исполнитель готовит вопросы, расчёт, варианты или письмо;
5. программа подставляет товар, цену и остатки своего склада и партнёров из
   сохранённого снимка и проверяет маршрут;
6. отдельный запуск модели из `OPENCODE_REVIEWER_MODEL` отмечает условие,
...
9. `GET /api/market` показывает текущие учебные предложения, а защищённый
...
Права остаются явными. Ведущий управляет учебными остатками, особый вариант
...
Стенд сохраняет последовательность наблюдаемых действий:
...
| Ведущий возвращает партнёру 260 кг | Система снова сравнивает все предложения и возвращает более быстрый вариант |
...
10. отдельный запуск модели из `OPENCODE_REVIEWER_MODEL` проверяет обещания,
    особые условия и границы полномочий; без переменной используется GPT-5.6
    Sol;
...
DeepSeek V4 Flash и DeepSeek V4 Pro доступны как рабочие модели, по умолчанию
выбрана DeepSeek V4 Flash. GPT-5.6 Sol доступна для сравнительного подробного
запуска. После исполнителя OpenCode отдельно запускает модель проверки из
`OPENCODE_REVIEWER_MODEL`; значение по умолчанию —
`opencode/gpt-5.6-sol`. При выборе GPT-5.6 Sol исполнителем и модели проверки
по умолчанию OpenCode запускает её дважды: для исполнения и проверки.
...
поставщиков, выбранный исполнитель готовит новое письмо, а отдельная модель из
`OPENCODE_REVIEWER_MODEL` проверяет обещания. Кнопка для изменения из другого
...
Для рабочих запусков доступны DeepSeek V4 Flash и DeepSeek V4 Pro; GPT-5.6
Sol доступна для сравнительного подробного запуска. После запуска исполнителя
отдельная модель из `OPENCODE_REVIEWER_MODEL` проверяет обещания, особые
условия и границы полномочий заказа. По умолчанию эту роль выполняет GPT-5.6
Sol.
...
`GET /api/market` соединяет его с текущими значениями D1. Ведущий меняет
...
- полный список учебных предложений партнёров с идентификатором, остатком,
...
если ведущий меняет предложение во время обработки следующей карточки.
...
длину текста и добавляет JSON к заданию выбранной текстовой модели, по
умолчанию DeepSeek V4 Flash. Инструкция запрещает подтверждать по наблюдению
...
склад. При остатке 420 из 620 кг он читает из D1 живые учебные предложения
других поставщиков. Для недостающих 200 кг агент:
...
Предложения живые в пределах стенда. Когда ведущий меняет остаток «Индустрии
...
Основной учебный сценарий использует один точный расчёт:

- клиенту нужно 620 кг КР-003;
- на складе «Колера» есть 420 кг по 415 ₽/кг:
  `420 × 415 = 174 300 ₽`;
- в живом учебном предложении «Индустрии Покрытий» указаны недостающие 200 кг по
  402 ₽/кг:
  `200 × 402 = 80 400 ₽`;
...
учебной строки и сохраняет границу: фактический резерв у поставщика появляется
...
ведущего. В рабочей системе агент сначала читает подключённые склады
...
Текущий стенд обновляет учебное предложение, готовит запрос подтверждения и
...
Публичный стенд не резервирует товар в учётной или складской системе. Для
...
- текущие изменяемые поля учебных предложений под ключом
...
| `GET /api/market` | Получить текущие учебные предложения других поставщиков |
| `PATCH /api/market` | Изменить доступный объём выбранного предложения из пульта ведущего |
...
- исполнитель выбирается из разрешённого списка;
...
- выбранный исполнитель ведёт заказ;
- модель из `OPENCODE_REVIEWER_MODEL` отдельным запуском проверяет результат;
  значение по умолчанию — GPT-5.6 Sol;
...
6. В пульте ведущего выберите «Индустрия Покрытий» и измените доступный объём
...
стенд не резервирует товар в учётной системе и не доставляет письмо.
...
Отдельная модель из `OPENCODE_REVIEWER_MODEL` проверит решение; по умолчанию
это GPT-5.6 Sol.
...
> Выбранная рабочая модель ведёт заказ по открытой инструкции. Программа
> проверяет числа, отдельная модель из `OPENCODE_REVIEWER_MODEL` проверяет
> факты и обещания, руководитель подтверждает один вариант.
...
- [ ] В пульте ведущего видны выбор поставщика, доступный объём и быстрые
...
- [ ] Ведущий проговаривает границу: инструкция MiMo требует считать текст на
...
- [ ] Отдельный запуск модели из `OPENCODE_REVIEWER_MODEL` выполняет проверку;
      без переменной используется GPT-5.6 Sol.
...
- Быстрый режим выполняет программные правила. Выбранный исполнитель и
  отдельная модель из `OPENCODE_REVIEWER_MODEL` участвуют только при активной
  связи OpenCode.
- Изменение через встроенный пульт записывает новый факт и автоматически
  ...
  выбранный исполнитель и отдельная модель проверки; без связи карточку
...
- D1 хранит текущий склад, живые учебные предложения других поставщиков,
...
|---|---|---|---|
| Входящая заявка обычными словами | Форма создаёт карточку из темы, текста, компании и сайта | Рабочая входящая почта остаётся следующим подключением | Выполнено в стенде |
...
| Агент обращается к живой системе | Каждая карточка получает из D1 один снимок своего склада и предложений других поставщиков | Данные учебные, изменение доступно ведущему | Выполнено |
...
| Рабочая модель следует открытой инструкции | DeepSeek V4 Flash получает инструкцию, программа сверяет числа с каталогом и складом, отдельная модель из `OPENCODE_REVIEWER_MODEL` проверяет обещания, особые условия и границы полномочий | Требуется активный локальный мост OpenCode; значение модели проверки по умолчанию — GPT-5.6 Sol | Выполнено в живом режиме |
...
| У системы есть «ручки и ножки» | Система читает из D1 свой склад и предложения других поставщиков, получает фото из R2, ищет и открывает общедоступные страницы, пишет результат, события, пересчёты и состояния карточки | Входящая и исходящая почта остаются следующим подключением | Выполнено в границах стенда |
