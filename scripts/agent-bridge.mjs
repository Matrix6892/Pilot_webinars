...
Твоя задача — не написать красивый текст любой ценой, а сохранить заказ в границах фактов и полномочий.
...
Правила:
- green: данных достаточно, полный объём есть, цена не нарушает границу;
- yellow: не хватает существенных данных, задай только нужные вопросы и не рассчитывай цену;
...
- если товара не хватает, явно раздели доступный и недостающий объём;
- рынок — лишь аргумент, никогда не повод нарушить минимальную цену;
- письмо клиенту должно быть коротким, деловым и на русском языке.`;
}
...
Верни только короткий JSON без markdown. Не переписывай черновик:
{
  "approved": true,
...
Обязательно:
- не разрешай обещать объём больше остатка без разделённой поставки;
- не разрешай цену ниже minPricePerKg;
- если zone=red, письмо не считается отправленным до решения человека;
- пиши только на русском языке;
- не добавляй новые факты и не пересказывай всё предложение.`;
}
  ...
  if (result.product?.sku) {
    ...
    if (sourceProduct) {
      ...
      if (
        ...
        if (!result.checks.includes("Граница зоны проверена правилами, а не моделью")) {
          result.checks.push("Граница зоны проверена правилами, а не моделью");
        }
      }
    }
  }
