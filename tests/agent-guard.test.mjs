...
test("compound estimates keep each ask distinct and surface grounded catalog guidance", () => {
  ...
  liveCoverageRepair.reply.body = [
    ...
    "1. Забор. Подходит КР-005 «Краска для дерева на улице»: защищает от дождя и помогает дереву не подхватить грибок. Расход 0,14 кг/м² на слой, кладём два слоя. Для типичного дачного забора длиной 30–90 м и высотой 1,5–2 м получается 45–180 м², краски 13–50 кг, по цене 296 ₽/кг — примерно 3 800–14 800 ₽. Склад такой объём подтверждает. Пришлёте точные длину и высоту — посчитаю до килограмма.",
    "2. Цвет забора. В палитре КР-005 три цвета: белый, коричневый, зелёный. Рекомендую коричневый: на дереве выглядит естественно, не маркий, неровности и потёки на нём менее заметны. Зелёный — если хотите, чтобы забор сливался с садом; белый — нарядный, но пыль и грязь на нём виднее.",
    "3. Кремль. Это объект культурного наследия федерального значения и Всемирного наследия ЮНЕСКО: стены протяжённостью 2235 м и высотой 5–19 м. Для честного масштаба: только стены (без башен) — 11–43 тыс. м², одного материала по обычным меркам вышло бы 3–12 т и 0,9–4,8 млн ₽, а полная реставрационная покраска с лесами, допусками и специалистами — в разы дороже. Это мысленная оценка масштаба, не расчёт и не оферта: ни один наш продукт под такой объект не заявлен.",
    "4. Цвет Кремля. Его выбирают не по палитре: цвет стен утверждают реставрационные регламенты и исторические исследования. По этому пункту честная рекомендация — обращаться к реставрационным организациям, а не к нашему каталогу.",
    "Итог: забор — КР-005, советую коричневый, краска обойдётся примерно в 3 800–14 800 ₽; по Кремлю — только масштабная прикидка, реальная стоимость определяется реставрационным процессом.",
  ].join("\n\n");
  const liveCoverageResult = normalizeV2AgentResult(
    liveCoverageRepair,
    demoData,
    job,
  );
  assert.match(
    liveCoverageResult.reply.body,
  ...
  const traditionalColorAdvice = structuredClone(result);
  traditionalColorAdvice.reply.body =
    "Цвет забора: рекомендую коричневый — классика для дачи. " +
    "А цвет Кремлю по традиции — красный, кирпичный, тот самый вид с Красной площади.";
  assert.deepEqual(compoundReplyCoverageGaps(job, traditionalColorAdvice), []);
