...
export function activeOrderProgress(events, startedAt, now = Date.now()) {
  ...
  const elapsed = minutes
    ...
    : `${seconds} сек`;
  const reviewing = Array.isArray(events) &&
    events.some((event) => String(event?.stage ?? "").startsWith("review"));
  const titles = [
    "Разбираю все части запроса",
    "Сверяю каталог, цены и доступные данные",
    "Собираю расчёт, рекомендации и полный ответ",
  ];

  return {
    title: reviewing
      ? "Проверяю, что ответ охватывает все просьбы"
      : titles[Math.floor(elapsedSeconds / 30) % titles.length],
    elapsed,
  };
}
