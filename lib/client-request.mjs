...
export function activeOrderProgress(events, startedAt, now = Date.now()) {
  const startedAtMs = timestampMs(startedAt);
  const elapsedSeconds = Math.max(
    0,
    Math.floor((now - (Number.isFinite(startedAtMs) ? startedAtMs : now)) / 1_000),
  );
  ...
  const stageTitles = {
    received: "Разбираю отдельные вопросы",
    understanding: "Разбираю отдельные вопросы",
    vision: "Изучаю фото",
    "vision-result": "Готовлю решение с учётом фото",
    primary: "Готовлю расчёт и подбор",
    research: "Проверяю открытый источник",
    "research-result": "Готовлю ответ по проверенному источнику",
    "primary-retry": "Дополняю пропущенную часть",
    review: "Проверяю полноту ответа",
    "review-result": "Фиксирую проверенный ответ",
    "review-fallback": "Сохраняю честный путь ручной проверки",
    decision: "Фиксирую итоговое решение",
  };
  const lastEventAt = lastEvent?.createdAt ?? startedAt;
  const lastEventAtMs = timestampMs(lastEventAt);
  const quietSeconds = Math.max(
    0,
    Math.floor(
      (now - (Number.isFinite(lastEventAtMs) ? lastEventAtMs : now)) / 1_000,
    ),
  );
  const detail =
    quietSeconds >= 180
      ...
      : quietSeconds >= 30
        ...
        : "Этап подтверждён системой; ждём следующее сохранённое событие.";

  return {
    title:
      stageTitles[stage] ??
      String(lastEvent?.title ?? "Продолжаю подтверждённый этап"),
    detail,
    elapsed,
    lastEventAt,
  };
