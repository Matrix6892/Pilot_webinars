...
const ORDER_STATUS_LABELS = {
  ...
  sent: "Система записала отправку",
  error: "Заказ сохранён · обработку можно запустить снова",
};
...
const MODEL_LABELS = {
  ...
  "deepseek/deepseek-v4-flash": "DeepSeek V4 Flash · API DeepSeek",
...
const EVENT_STAGE_LABELS = {
  ...
  "review-result": "Проверка DeepSeek V4 Pro завершена",
  "review-fallback": "Передача решения руководителю",
  "review-skipped": "Безопасное завершение без повторной проверки",
  ...
  error: "Заказ можно запустить снова",
  agent: "Работа агента",
...
const EVENT_STAGE_PRESENTATIONS = {
  ...
  "review-skipped": {
    recordType: "Безопасное завершение",
    sender: "Программа проверки",
    recipient: "Руководитель",
    stateLabel: "Fail-closed результат передан руководителю",
  },
  vision: {
  ...
  "clarification-sent": {
    ...
    sender: "Агент отдела продаж",
    recipient: "Клиент",
    stateLabel: "Система записала вопросы",
  },
  "customer-reply": {
  ...
  sent: {
    ...
    sender: "Агент отдела продаж",
    recipient: "Клиент",
    stateLabel: "Система записала отправку ответа",
  },
  approval: {
...
function hasStoredResultJson(value) {
  return typeof value === "string" && value.trim().length > 0;
}

export function decodeStoredAgentResultForLedger(value) {
  const parsed = storedJson(value);
  ...
  if (isCompleteAgentResult(decoded)) return decoded;
  return !Object.hasOwn(parsed, "schemaVersion") ? decoded : null;
}

export function projectLedgerOrder(order) {
  return {
    ...order,
    result: hasStoredResultJson(order.resultJson)
      ? decodeStoredAgentResultForLedger(order.resultJson)
      : objectValue(order.result),
    resultJson: undefined,
  };
}

export function projectLedgerResultHistory(history) {
  const hasResult = hasStoredResultJson(history.resultJson);
  const result = hasResult
    ? decodeStoredAgentResultForLedger(history.resultJson)
    : objectValue(history.result);
  return {
    ...history,
    replySubject: result?.reply?.subject ??
      (hasResult ? "" : textValue(history.replySubject)),
    replyBody: result?.reply?.body ??
      (hasResult ? "" : textValue(history.replyBody)),
    zoneReason: result?.zoneReason ??
      (hasResult ? "" : textValue(history.zoneReason)),
    optionsJson: result
      ? JSON.stringify(result.options ?? [])
      : hasResult
        ? "[]"
        : textValue(history.optionsJson),
    result,
    resultJson: undefined,
  };
}
...
function resultFromOrder(order) {
  return hasStoredResultJson(order.resultJson)
    ? decodeStoredAgentResultForLedger(order.resultJson)
    : objectValue(order.result);
}
...
function friendlyLedgerText(value) {
  return textValue(value)
    ...
    .replace(/учебн(?:ая|ую|ой)\s+таблиц\p{L}*\s+цен/giu, (match) =>
      replacementWithInitialCase(match, "таблица поставщиков"),
    )
    .replace(
    ...
    .replace(/карточк(?:а|е|у|ой)\s+заказа/giu, (match) =>
...
function orderRecords(order, includeCurrentResult = true) {
  ...
  const result = resultFromOrder(order);
...
function resultHistoryRecord(history, ordersById) {
  const projected = projectLedgerResultHistory(history);
  const options = storedJson(projected.optionsJson);
  ...
  const values = {
    ...
    "Состояние заказа": cardStatus(history.status),
    ...
    Отправитель: "Агент отдела продаж",
    Получатель: "Страница заказа",
    "Тема письма": textValue(projected.replySubject),
    "Полный текст": friendlyLedgerText(projected.replyBody),
    ...
    "Основание решения": textValue(projected.zoneReason),
