...
const ORDER_STATUS_LABELS = {
  ...
  awaiting_customer: "Ожидается ответ клиента",
  awaiting_approval: "Ожидается решение руководителя",
  ready_to_send: "Готово к отправке",
  completed: "Готово к отправке",
  sent: "Отправка записана на стенде",
  error: "Требуется повторный запуск",
};

const EVENT_STATE_LABELS = {
  active: "В работе",
  done: "Завершено",
  error: "Нужен повторный запуск",
...
const EVENT_STAGE_LABELS = {
  ...
  "review-result": "Проверка сильной моделью завершена",
...
const EVENT_STAGE_PRESENTATIONS = {
  ...
  "review-fallback": {
    ...
    stateLabel: "Передано руководителю",
  },
  vision: {
  ...
  "vision-fallback": {
    ...
    sender: "Программа стенда",
    recipient: "Агент отдела продаж",
    stateLabel: "Продолжено с уточняющими вопросами",
  },
  model: {
...
function orderRecords(order, includeCurrentResult = true) {
  ...
  if (Array.isArray(conversation)) {
    conversation.forEach((message, index) => {
      ...
      const conversationRecord = {
        ...
        "Состояние записи": customer ? "Получено" : "Отправлено на стенде",
        "Состояние карточки": status,
  ...
  if (includeCurrentResult && (replyBody || textValue(reply?.subject))) {
    ...
    const agentEmail = {
      ...
      "Состояние записи": sentAt ? "Отправлено на стенде" : "Подготовлено",
      "Состояние карточки": status,
  ...
  if (managerDecision) {
    ...
    const decisionRecord = {
      ...
      "Состояние записи": "Решение принято",
      "Состояние карточки": status,
      ...
      "Полный текст": managerDecision,
      "Решение руководителя": managerDecision,
...
function resultHistoryRecord(history, ordersById) {
  ...
  const values = {
    ...
    "Состояние записи": textValue(history.reason) || "Решение сохранено",
    ...
    "Полный текст": textValue(history.replyBody),
    "Название события": textValue(history.reason),
...
function inventoryRecord(change) {
  ...
  const values = {
    ...
    "Причина изменения склада": textValue(change.reason),
    "Кто изменил склад": textValue(change.actor),
    ...
    ...records.map(({ values }) =>
      csvLine(
        LEDGER_CSV_HEADERS.map((header) =>
          ["Дата и время", "Время решения руководителя", "Время отправки"].includes(
            header,
          )
            ? ledgerDateTime(values[header])
            : values[header],
        ),
      ),
    ),
