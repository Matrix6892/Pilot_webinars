...
const CLIENT_COLOR_CODE_PATTERN =
  /\bRAL\s?-?\s?\d{4}\b|(?:код\p{L}*\s+цвет\p{L}*)\s*[-—:]?\s*\d{3,4}/giu;

function scrubClientColorCodes(text) {
  return typeof text === "string"
    ? text.replace(CLIENT_COLOR_CODE_PATTERN, "тон по образцу")
    : text;
}

function scrubReplyColorCodes(reply) {
  if (!reply || typeof reply !== "object") return reply;
  const next = { ...reply };
  if (typeof next.subject === "string") {
    next.subject = scrubClientColorCodes(next.subject);
  }
  if (typeof next.body === "string") {
    next.body = scrubClientColorCodes(next.body);
  }
  return next;
}
...
const colorRecommendationPattern =
  /(?:рекоменд|посовет|совет(?:ую|уем|уете|уют|овал\p{L}*|овать)|выбрал|выбран|выбрать|выбир\p{L}*|предпоч|практичн|лучше|стоит\s+сохран\p{L}*|(?:правильн|лучш|оптимальн)\p{L}*\s+вариант[^.!?\n]{0,60}сохран\p{L}*|облик[^.!?\n]{0,80}утвержд\p{L}*[^.!?\n]{0,200}перекраск\p{L}*[^.!?\n]{0,80}не\s+соглас\p{L}*|(?:цвет|палитр)\p{L}*[^.!?\n]{0,60}по\s+традици\p{L}*|сохран\p{L}*[^.!?\n]{0,60}(?:палитр|цвет|облик)|\b(?:recommend|choose|prefer|practical)\b|worth\s+preserv\p{L}*|colou?r[^.!?\n]{0,60}(?:by\s+)?tradition|preserv\p{L}*[^.!?\n]{0,60}(?:palette|appearance))/iu;
const colorChoicePattern =
  /(?:коричнев|зел[её]н|бел|красн|сер|ч[её]рн|палитр|оттен|облик|\bRAL\b|\bcolou?r\b|\bshade\b|\bappearance\b)/iu;
const colorHeadingPattern =
  /^(?:цвет|палитр|оттен)\p{L}*(?:\s+для)?(?:\s|$)/iu;
const protectedCulturalEvidencePattern =
  /(?:всемирн\p{L}*\s+наслед|культурн\p{L}*\s+наслед|юнеско|\bunesco\b|памятник\p{L}*|реставрац\p{L}*)/iu;
const askCatalogRequestPattern =
  /(?:каталог\p{L}*|артикул\p{L}*|sku|какую\s+краск\p{L}*|како\p{L}*\s+покрыт\p{L}*|\b(?:catalog|product)\b)/iu;
const askPreparationRequestPattern =
  /(?:подготов\p{L}*[^.!?\n]{0,80}(?:поверхност|основан)|(?:поверхност|основан)\p{L}*[^.!?\n]{0,80}подготов\p{L}*|подготов\p{L}*[^.!?\n]{0,100}(?:перед|к)\s+(?:покраск|окраск|нанесени\p{L}*\s+краск|покрыти)\p{L}*|\b(?:prepare|preparation)\b[^.!?\n]{0,80}\b(?:surface|substrate)\b|\bprepare\b[^.!?\n]{0,100}\bbefore\s+(?:paint|coating))/iu;
const preparationActionPattern =
  /(?:очист\p{L}*|обезжир\p{L}*|отшлиф\p{L}*|зашкур\p{L}*|(?:за)?грунт\p{L}*|просуш\p{L}*|удал\p{L}*[^.!?\n]{0,40}(?:пыл|гряз|стар\p{L}*\s+покрыт)|\b(?:clean|degreas|sand|prime|dry)\p{L}*)/iu;
...
function groundedEstimateMethod(method, intent, job) {
  const candidate = cleanSemanticCopy(method)
    .match(/^(.{1,80}?)(?::|[—–-]\s)/u)?.[1]
    ?.trim();
  const trustedRoots = new Set(lexicalRoots(latestCustomerText(job)));
  const candidateRoots = askScopeRoots(candidate);
  const scope = candidate &&
      candidateRoots.length > 0 &&
      candidateRoots.every((root) => trustedRoots.has(root))
    ? candidate
    : cleanSemanticCopy(
  ...
  return `${scope || "Объект из запроса"}: диапазонная оценка по подтверждённым данным заявки.`;
}
...
export function askCoverageGaps(job, result) {
  ...
  const authoredCovers = (ask) => {
    ...
    const requiredFacets = [
      ...
      askPreparationRequestPattern.test(ask.request)
        ? askPreparationRequestPattern
        : null,
    ].filter(Boolean);
    if (
  ...
  const colorGuidanceParts = visibleText
    ...
    .map((part) => part.trim())
    .filter(Boolean);
  const sectionColorGuidance = visibleText
    .split(/\n{2,}/u)
    .flatMap((paragraph) => {
      let section = "";
      return paragraph.split(/[.!?]+/u).flatMap((part) => {
        const candidate = part.trim();
        if (/^(?:по|for)\s+.{2,100}[—:]/iu.test(candidate)) {
          section = candidate;
        }
        return section &&
            colorRecommendationPattern.test(candidate) &&
            colorChoicePattern.test(candidate)
          ? [`${section} ${candidate}`]
          : [];
      });
    });
  const colorGuidanceRoots = [
    ...colorGuidanceParts
    .map((part, index) => {
      ...
      if (
        directGuidance &&
        index > 0 &&
        /(?:\b(?:его|её|там)\b|облик)/iu.test(part)
      ) {
        // ponytail: the previous sentence supplies the object for "его/облик".
        return `${colorGuidanceParts[index - 1]} ${part}`;
      }
      return part.length <= 100 && colorHeadingPattern.test(part) && !directGuidance
    ...
    // ponytail: section paragraphs keep the named object for later "там" references.
    ...sectionColorGuidance,
  ]
  ...
  return asks
    .filter((ask) => {
      ...
      const requestsPreparation = askPreparationRequestPattern.test(ask.request);
      ...
      if (requestsPreparation && !preparationActionPattern.test(visibleText)) {
        return true;
      }
...
function canonicalPlanningEstimates(estimates, intent, demoData, job) {
  ...
  if (
    ...
    (!fencePlanning &&
      (estimates.length > 0 ||
        !askCostRequestPattern.test(request)))
  ) {
    return estimates;
  }
  const usableProducts = (demoData?.products ?? []).filter(
    (item) =>
  ...
  const targetRoots = new Set(lexicalRoots(intent.target.label));
  const exactProducts = usableProducts.filter((product) =>
    productSupportsIntent(product, intent),
  );
  const substrateProducts = usableProducts.filter((product) =>
    product.substrates?.some((substrate) =>
      lexicalRoots(substrate).some((root) => targetRoots.has(root)),
    ),
  );
  const compatibleProducts = exactProducts.length
    ? exactProducts
    : substrateProducts;
  const product = fencePlanning
    ? usableProducts.find((item) => item.sku === "КР-005")
    : compatibleProducts.length === 1
      ? compatibleProducts[0]
      : null;
  if (!product) return estimates;

  const target = cleanSemanticCopy(intent.target.label).slice(0, 80);
  const areaMin = fencePlanning ? 20 : 10;
  const areaMax = fencePlanning ? 60 : 50;
  const scope = fencePlanning ? "Дачный забор" : target;
  ...
  let assumption = intent.assumptions.find(
    (item) =>
      (fencePlanning
        ...
        : lexicalRoots(item.claim).some((root) => targetRoots.has(root))) &&
  ...
  if (!assumption) {
    const baseId = fencePlanning ? "fence-planning-range" : "planning-range";
    let id = baseId;
    ...
    assumption = {
      id,
      claim: `До получения размеров «${target}» используем плановый диапазон площади ${areaMin}–${areaMax} м².`,
      basedOnEvidenceIds: evidenceIds,
      confidence: 0.35,
      range: { min: areaMin, max: areaMax, unit: "м²" },
  ...
  const priorByMetric = new Map(
    estimates
      .filter((item) =>
        lexicalRoots(item.method).some((root) => targetRoots.has(root)),
      )
  ...
  const planningEstimates = [
    estimate(
      "surface_area",
      { min: areaMin, max: areaMax, unit: "м²" },
      `${scope}: плановый диапазон площади до получения размеров`,
    ),
    estimate(
      "paint_quantity",
      { min: paintMin, max: paintMax, unit: "кг" },
      `${scope}: площадь × расход каталога × ${product.recommendedCoats} слоя с 10% запаса`,
    ),
    estimate(
      "budget",
      { min: budgetMin, max: budgetMax, unit: "₽" },
      `${scope}: расход округлён до целых упаковок × цена каталога`,
    ),
  ];
  if (!fencePlanning) return planningEstimates;
  return [
    ...planningEstimates,
...
function normalizeV2Envelope(value, demoData, job) {
  ...
  const estimates = canonicalPlanningEstimates(
    ...
    reply: {
      ...
      body: suppressUnsupportedAttribution
        ...
        : safeAuthoredFragments(
            value.reply?.body,
            demoData?.products ?? [],
            16, // ponytail: one bounded full reply; unsafe fragments are still filtered.
          ).join(" "),
    },
    allEvidenceGrounded:
      rawEvidenceCount > 0 &&
...
const rhetoricalSingleRecommendationPattern =
  /(?:рекомендац\p{L}*|совет\p{L}*)\s+(?:один|одна|одно)(?=$|[^\p{L}])/iu;
...
function cleanAuthoredCopy(value) {
  // ponytail: keep the positive clause while preserving the "не X, а Y" copy rule.
  const positiveCopy = text(value)
    .replace(
      /[:;][^.!?\n]{0,160}\sне\s[^.!?\n]{0,120},\s*а\s+[^.!?\n]+(?=[.!?](?:\s|$))/giu,
      "",
    )
    .replace(/,\s*а\s+не\s+[^.!?\n]+(?=[.!?](?:\s|$))/giu, "");
  return cleanClientCopy(positiveCopy)
    .replace(leadingVocativePattern, "")
    .trim();
}
  ...
  return candidates
    ...
    .filter(
      (item) =>
        ...
        // ponytail: "рекомендация одна" is rhetoric; any other number still drops the sentence.
        !authoredNumberPattern.test(
          item.replace(rhetoricalSingleRecommendationPattern, ""),
        ) &&
...
function v2BoundaryReply(
  ...
  return {
    ...
    body: `Понял: нужен рабочий результат для задачи «${targetLabel}». Сейчас строительное покрытие нельзя рекомендовать без подтверждения совместимости. Его пригодность пока не подтверждена. Для зон с открытым огнём, контактом с пищей или нагревом совместимость строительного покрытия подтверждается только документами производителя. ${firstStep}${verifyTemperature ? " Проверим допустимую рабочую температуру и область применения по документам до рекомендации." : ""} Предлагаю параллельно проверить каталог и подготовить подходящую альтернативу, чтобы сохранить требуемый результат без неподтверждённого обещания; ограничение — при отсутствии совместимой системы понадобится специализированный вариант.`,
  };
}
...
function rangeEstimateReply(
  ...
  const preparationAdvice =
    askPreparationRequestPattern.test(requestText) &&
    !preparationActionPattern.test(authoredBody)
      ? `По подготовке «${target}»: очистите поверхность от грязи и непрочного старого покрытия, удалите пыль и просушите; шлифование и совместимый грунт выберите после проверки состояния основания.`
      : "";
  const colorAdvice = asksForColorAdvice &&
    ...
    : "";
  const authoredColorParts = String(authoredBody ?? "").split(/[.!?\n]+/u);
  const protectedClaims = (resolvedIntent?.evidence ?? [])
    .filter(
      (item) =>
        item?.source?.kind === "web" &&
        protectedCulturalEvidencePattern.test(item?.claim ?? ""),
    )
    .map((item) => new Set(lexicalRoots(item.claim)));
  const protectedColorAdvice = asksForColorAdvice
    ? rangeGroups.flatMap(({ scope }) => {
        const scopeRoots = lexicalRoots(scope);
        const grounded =
          scopeRoots.length > 0 &&
          protectedClaims.some((roots) =>
            scopeRoots.some((root) => roots.has(root)),
          );
        const alreadyCovered = authoredColorParts.some((part) => {
          const roots = new Set(lexicalRoots(part));
          return (
            scopeRoots.some((root) => roots.has(root)) &&
            colorRecommendationPattern.test(part) &&
            colorChoicePattern.test(part)
          );
        });
        // ponytail: trusted heritage evidence gets one safe, object-scoped answer.
        return grounded && !alreadyCovered
          ? [`Для «${scope}» рекомендую сохранить утверждённый исторический облик; точный оттенок должен определять профильный реставрационный проект.`]
          : [];
      })
    : [];
...
export function normalizeV2AgentResult(value, demoData, job = {}) {
  ...
  // Клиент не видит внутренних кодов цвета: тон фиксируется образцом или
  // пробным выкрасом; каталоговые RAL-коды модель иногда копирует в текст.
  value.reply = scrubReplyColorCodes(value.reply);
  if (Array.isArray(value.options)) {
    value.options = value.options.map((option) =>
      option && typeof option === "object"
        ? { ...option, reply: scrubReplyColorCodes(option.reply) }
        : option,
    );
  }
  ...
  const rawRequestedCommitment = [
    ...
    ? value.commitment
    : "none";
  const hasCanonicalPlanningEstimate = envelope.estimates.some((estimate) =>
    estimate.assumptionIds?.some((id) =>
      /^(?:fence-)?planning-range(?:-|$)/u.test(id),
    ),
  );
  const requestedCommitment =
    rawRequestedCommitment === "none" && hasCanonicalPlanningEstimate
      ? "estimate"
      : rawRequestedCommitment;
