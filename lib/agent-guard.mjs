...
function groundedUnderstanding(job, facts) {
  ...
  const understood = [
    ...
    facts.matchedProduct
      ? `Продукт по каталогу: ${facts.matchedProduct.name} (${facts.matchedProduct.sku})`
      : "",
    facts.requestedKg
  ...
  return understood.length
    ? understood
    : ["Письмо пока не содержит подтверждённых параметров для расчёта."];
}
...
export function normalizeAgentResult(result, demoData, job = {}) {
  ...
  const sourceFactGaps = [...facts.gaps];
  ...
  if (modelSkuMismatch) {
    ...
    sourceFactGaps.unshift(
      `подтвердите артикул: модель указала ${modelSkuLabel}, письмо соответствует ${matchedProduct.sku}`,
    );
  }
...
export function applyReviewerResult(
  ...
  const mismatchNote = normalized.missing.find((item) =>
    /^подтвердите артикул: модель указала (?:КР-\d{1,4}|другой артикул), письмо соответствует КР-\d{1,4}$/iu.test(
      item,
    ),
  );
  ...
  normalized.managerNote =
    "Руководитель видит замечания ИИ-рецензента и выбирает безопасный следующий ход.";
  ...
  normalized.businessContext = hasProduct
    ? hasResearchSources
      ? "ИИ-рецензент исключил неподтверждённые выводы. Письмо, каталог, остатки и открытые источники дают контекст для решения."
      : "ИИ-рецензент исключил неподтверждённые выводы. Письмо, каталог и остатки дают контекст для решения."
    : hasResearchSources
      ? "ИИ-рецензент исключил неподтверждённые выводы. Письмо и открытые источники дают контекст для решения."
      : "ИИ-рецензент исключил неподтверждённые выводы. Письмо задаёт контекст для следующего шага.";
  if (normalized.research) {
    ...
    normalized.research.summary = hasResearchSources
      ...
      : normalized.research.checked
        ? "Публичный поиск завершён. Подтверждённых фактов для решения агент не нашёл."
        : "";
  }
  normalized.options = [
    {
