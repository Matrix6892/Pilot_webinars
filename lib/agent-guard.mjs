...
const colorRecommendationPattern =
  /(?:рекоменд|посовет|совет(?:ую|уем|уете|уют|овал\p{L}*|овать)|выбрал|выбран|выбрать|выбир\p{L}*|предпоч|практичн|лучше|(?:цвет|палитр)\p{L}*[^.!?\n]{0,60}по\s+традици\p{L}*|сохран\p{L}*[^.!?\n]{0,60}(?:палитр|цвет|облик)|\b(?:recommend|choose|prefer|practical)\b|colou?r[^.!?\n]{0,60}(?:by\s+)?tradition|preserv\p{L}*[^.!?\n]{0,60}(?:palette|appearance))/iu;
...
function cleanAuthoredCopy(value) {
  ...
  const positiveCopy = text(value).replace(
    /,\s*а\s+не\s+[^.!?\n]+(?=[.!?](?:\s|$))/giu,
    "",
  );
