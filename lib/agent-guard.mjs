...
function orderColorLabel(value) {
  ...
  const colorName = colorNames.find(([pattern]) => pattern.test(source))?.[1];
  const ral = source.match(/\bRAL\s*\d{4}\b/iu)?.[0].replace(/\s+/g, " ");
...
function marketSnapshot(product, demoData) {
  ...
  const position =
    marketAverage === 0
      ? `Цена ${money.format(
          product.pricePerKg,
        )} ₽/кг подтверждена карточкой товара. Завод зарабатывает при цене от ${money.format(
  ...
  const profitOpportunity =
    difference > 0
      ...
      : "По текущей цене завод зарабатывает на заказе. Заработать больше помогут крупный заказ или удобный график поставки.";
  ...
  if (alternative) {
    options.push({
      ...
      rationale: `${money.format(
        alternative.stockKg,
      )} кг есть на складе. Агент попросит специалиста подтвердить, что краска подходит для задачи.`,
      tradeoff: `Цена составляет ${money.format(
        alternative.pricePerKg,
      )} ₽/кг. Специалист подтвердит, что краска подходит вашей поверхности.`,
      reply: `Можем предложить ${alternative.name} из наличия в полном объёме. Цена составляет ${money.format(
        alternative.pricePerKg,
      )} ₽/кг. Специалист подтвердит, что краска подходит; затем склад отложит нужный объём.`,
    });
  }
...
function fallbackReply(result, job) {
  ...
  if (result.product) {
    const sourceOrderText = orderText(job);
    const color = orderColorLabel(sourceOrderText);
...
export function normalizeAgentResult(result, demoData, job = {}) {
  if (!result || typeof result !== "object") {
    throw new Error(
      "Ответ модели нужно пересобрать. Запустите карточку снова.",
    );
  }
  ...
  result.businessContext =
    openedResearchSources.length > 0
      ? openedResearchSources.map((source) => source.fact).join(" ")
      : !hadResearchClaim && suppliedBusinessContext
  ...
  for (const source of openedResearchSources) {
    groundedBasis.push({
      fact: source.fact,
      source: source.title,
  ...
  const suppliedReply = result.reply && {
    ...
    body: allResearchClaimsGrounded ? cleanCopy(result.reply.body) : "",
  };
  result.reply =
    result.zone === "yellow"
...
export function applyReviewerResult(
  ...
  const readyProduct = result.product
    ...
    : null;
  const readyDetails = [
    orderColorLabel(facts.sourceOrderText) ? "цвет" : "",
  ...
  if (canAskClient) {
    ...
    result.businessContext = facts.matchedProduct
      ? "Письмо и каталог собраны. Ответ клиента завершит расчёт по живому складу."
      : "Письмо разобрано. Агент подготовил вопросы, после которых подберёт краску и рассчитает заказ.";
    result.zoneReason =
      "Агент собрал известные данные и подготовил короткие вопросы клиенту.";
    result.managerNote =
      "Отправьте точные вопросы. Ответ клиента продолжит ту же карточку.";
    ...
    if (hasResearchSources) {
      ...
      if (normalized.decisionBasis.length === 0) {
        normalized.decisionBasis = [
          {
            fact: "Запрос клиента сохранён в карточке.",
