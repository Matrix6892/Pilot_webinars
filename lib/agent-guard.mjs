...
const openedUnsupportedClaimPattern =
  /(?:\bRAL\b|КР-\d{1,4}|\b(?:SKU|артикул\p{L}*|код\p{L}*|food[- ]?safe|food[- ]?contact|non[- ]?toxic|heat[- ]?resistant|stock|price|delivery|guarantee)\b|₽|(?:^|[^\p{L}])(?:руб\p{L}*|цен\p{L}*|стоимост\p{L}*|налич\p{L}*|склад\p{L}*|остат\p{L}*|достав\p{L}*|постав\p{L}*|срок\p{L}*|гарант\p{L}*|обеспеч\p{L}*|пищев\p{L}*|термостойк\p{L}*|жаростойк\p{L}*|безопас\p{L}*|токсич\p{L}*)(?=$|[^\p{L}]))/iu;
...
function customerBridgeSupportsClaim(claim, excerpt, title, job) {
  ...
  const customerRoots = new Set(evidenceRoots(latestCustomerText(job)));
  const pageRoots = new Set([...titleRoots, ...excerptRoots]);
  return (
    claimRoots.size >= 2 &&
    titleRoots.size >= 2 &&
    [...claimRoots].filter((root) => customerRoots.has(root)).length >= 2 &&
...
const compoundPaintActionPattern =
  /(?:покрас\p{L}*|перекрас\p{L}*|окрас\p{L}*|\bpaint(?:ing|ed|s)?\b)/giu;
const compoundCostRequestPattern =
  /(?:сколько[^.!?;\n]{0,80}(?:стоит|стоить|обойд(?:е|ё)тся)|стоимост\p{L}*|бюджет\p{L}*|(?:^|[^\p{L}])цен(?:а|ы|у|е|ой)(?=$|[^\p{L}])|\b(?:how\s+much|cost|budget|price)\b)/iu;
const compoundCostBeforeActionPattern =
  /(?:сколько[^.!?;,\n]{0,60}(?:стоит|стоить|обойд(?:е|ё)тся)|(?:стоимост\p{L}*|бюджет\p{L}*|цен(?:а|ы|у|е|ой)|\b(?:how\s+much|cost|budget|price)\b)[^.!?;,\n]{0,40})\s*$/iu;
const compoundCoverageStopRoots = new Set(
  [
    "оценить",
    "покрасить",
    "окрасить",
...
export function compoundReplyCoverageGaps(job, result) {
  const request = latestCustomerText(job);
  const actionMatches = [...request.matchAll(compoundPaintActionPattern)];
  if (actionMatches.length < 2) return [];
  const requestParts = actionMatches.map((match, index) => ({
    index,
    part: cleanSemanticCopy(
      request.slice(
        match.index,
        actionMatches[index + 1]?.index ?? request.length,
      ),
    ).slice(0, 240),
    match,
  }));
  const partRoots = requestParts.map(({ part }) =>
    lexicalRoots(part).filter((root) => !compoundCoverageStopRoots.has(root)),
  );
  const goal = cleanSemanticCopy(result?.resolvedIntent?.goal);
  const goalParts = goal
    .split(/\s+(?:и|а\s+также|также|ещ[её])\s+/iu)
    .map((part) => part.trim())
    .filter(Boolean);

  const replyRoots = new Set(lexicalRoots(text(result?.reply?.body)));
  ...
  return requestParts
    .filter(({ index, part, match }) => {
      const distinguishingRoots = partRoots[index];
      const uniqueRoots = distinguishingRoots.filter((root) =>
        partRoots.every((otherRoots, otherIndex) =>
          otherIndex === index ? true : !otherRoots.includes(root),
        ),
      );
      const scopeRoots = uniqueRoots.length ? uniqueRoots : distinguishingRoots;
      const beforeAction = request.slice(
        Math.max(0, match.index - 120),
        match.index,
      );
      const trailingCostBelongsToNextAction = Boolean(
        actionMatches[index + 1] && compoundCostBeforeActionPattern.test(part),
      );
      // ponytail: a trailing "сколько стоит" belongs to the paint verb that follows it.
      const requestsBudget =
        compoundCostBeforeActionPattern.test(beforeAction) ||
        (!trailingCostBelongsToNextAction &&
          compoundCostRequestPattern.test(part));
      const hasScopedBudget =
        !requestsBudget ||
        budgetMethodRoots.some((methodRoots) =>
          scopeRoots.some((root) => methodRoots.has(root)),
        );
      return (
        distinguishingRoots.length > 0 &&
        (!scopeRoots.some((root) => replyRoots.has(root)) || !hasScopedBudget)
      );
    })
    .map(({ index, part }) => {
      const roots = new Set(partRoots[index]);
      return (
        goalParts.find((goalPart) =>
          lexicalRoots(goalPart).some(
            (root) => !compoundCoverageStopRoots.has(root) && roots.has(root),
          ),
        ) ?? part
      );
    })
...
export function decodeStoredAgentResult(value) {
  ...
  if (stored.schemaVersion === 2) {
    return isCompleteAgentResult(stored)
      ? stored
...
function rangeEstimateReply(
  ...
  const ranges = estimates.map((estimate) => {
    const scope =
      metricCounts.get(estimate.metric) > 1
        ? `${cleanAuthoredCopy(estimate.method)}: `
        : "";
    return `${scope}${labels[estimate.metric]} — ${decimal.format(
      estimate.range.min,
    )}–${decimal.format(estimate.range.max)} ${estimate.range.unit}`;
  });
  ...
  const catalogLine = candidateProduct
    ...
    : "";
  const asksForColorAdvice =
    /(?:како\p{L}*\s+цвет\p{L}*[^.!?]{0,60}(?:выбрать|лучше)|цвет\p{L}*[^.!?]{0,60}(?:посовет|рекоменд|лучше)|recommend[^.!?]{0,40}colou?r)/iu.test(
      requestText,
    );
  const preferredColor = candidateProduct
    ...
    : "";
  const colorAdvice = asksForColorAdvice && preferredColor
