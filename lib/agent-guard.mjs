...
function requestGroundedByMessage(request, evidenceItems) {
  ...
  return evidenceItems.some((item) => {
    ...
    const requestRoots = [...new Set(lexicalRoots(normalizedRequest))];
    const quoteRoots = new Set(lexicalRoots(quote));
    const matchingRoots = requestRoots.filter((root) => quoteRoots.has(root));
    return requestRoots.length > 0 && matchingRoots.length === requestRoots.length;
  });
}
...
const askColorRequestPattern =
  /(?:како\p{L}*\s+цвет|цвет\p{L}*\s+(?:выб|подобр|посовет)|палитр\p{L}*|оттен\p{L}*|\b(?:color|colour|palette|shade)\b)/iu;
...
export function askCoverageGaps(job, result) {
  ...
  const authoredKeys = new Set(authored.filter(Boolean).map(descriptor).map(descriptorKey));
  const candidates = [...authored, ...explicitRequestParts(latestCustomerText(job))]
    .filter(Boolean)
    .map(descriptor);
  const seen = new Set();
  const asks = candidates.filter((ask) => {
  ...
  const hasColorGuidance = askColorRequestPattern.test(visibleText) ||
    /(?:коричнев|зел[её]н|бел|красн|сер|ч[её]рн|палитр|оттен|\bRAL\b)/iu.test(visibleText);
  ...
  return asks
    .filter((ask) => {
      ...
      if (kind === "color") return !hasColorGuidance;
      if (kind === "catalog") return !hasCatalogGuidance;
      const scopeCovered =
        scopeRoots.length === 0 ||
        scopeRoots.some((root) => visibleRoots.has(root));
...
function canonicalFencePlanningEstimates(estimates, intent, demoData, job) {
  ...
  let assumption = intent.assumptions.find((item) =>
