...
const colorRecommendationPattern =
  /(?:рекоменд|посовет|совет(?:ую|уем|уете|уют|овал\p{L}*|овать)|выбрал|выбран|выбрать|выбир\p{L}*|предпоч|практичн|традиционн|традици\p{L}*|каноническ|лучше|сохран\p{L}*[^.!?\n]{0,60}(?:палитр|цвет)|\b(?:recommend|choose|prefer|practical|traditional|tradition|canonical)\b|preserv\p{L}*[^.!?\n]{0,60}palette)/iu;
    ...
    reply: {
      ...
      body: suppressUnsupportedAttribution
        ...
        : safeAuthoredFragments(
            value.reply?.body,
            demoData?.products ?? [],
            8, // ponytail: enough for one complete compound reply; unsafe fragments are still filtered.
          ).join(" "),
    },
    allEvidenceGrounded:
      rawEvidenceCount > 0 &&
