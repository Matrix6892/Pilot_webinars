...
export function askCoverageGaps(job, result) {
  ...
  const hasColorGuidance =
    /(?:рекоменд|посовет|совет(?:ую|уем|уете|уют|овал\p{L}*|овать)|выбрал|выбрать|предпоч|практичн|лучше|сохран\p{L}*[^.!?\n]{0,60}палитр|\b(?:recommend|choose|prefer|practical)\b|preserv\p{L}*[^.!?\n]{0,60}palette)/iu.test(visibleText) &&
  ...
  return asks
    .filter((ask) => {
      ...
      if (askColorRequestPattern.test(ask.request) && !hasColorGuidance) return true;
