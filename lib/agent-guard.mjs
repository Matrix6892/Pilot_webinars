...
export function askCoverageGaps(job, result) {
  ...
  const hasColorGuidance =
    /(?:рекоменд|посовет|выбрал|выбрать|предпоч|практичн|лучше|сохран\p{L}*[^.!?\n]{0,60}палитр|\b(?:recommend|choose|prefer|practical)\b|preserv\p{L}*[^.!?\n]{0,60}palette)/iu.test(visibleText) &&
    /(?:коричнев|зел[её]н|бел|красн|сер|ч[её]рн|палитр|оттен|\bRAL\b|\bcolou?r\b|\bshade\b)/iu.test(visibleText);
