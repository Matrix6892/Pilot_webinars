  ...
  const estimates = value
    .map((item) => {
      ...
      const candidateSkuGrounded = Boolean(
        candidateProduct && productSupportsIntent(candidateProduct, intent),
      );
...
export function managerFallbackAgentResult(
  ...
  const safeReviewerModel =
    cleanSourceText(reviewerModel, 120) || "opencode-go/deepseek-v4-pro";
...
function rangeEstimateReply(estimates, targetLabel = "объекта") {
  const labels = {
    surface_area: "площадь",
    paint_quantity: "краска",
    budget: "бюджет",
  };
  const ranges = estimates.map(
    (estimate) =>
      `${labels[estimate.metric]} — ${decimal.format(
        estimate.range.min,
  ...
  return {
    ...
    body: `Зафиксировал «${target}». ${ranges.join(
      "; ",
    )}. Это диапазонная оценка. Я предлагаю такой шаг: подготовим пробный выкрас для «${target}», чтобы проверить совместимость системы при фактическом освещении; точные коммерческие условия подтвердим перед предложением.`,
  };
}
...
export function normalizeV2AgentResult(value, demoData, job = {}) {
  ...
  if (managerEstimate) {
    return {
      ...
      reply: replyWithAuthoredLead(
        rangeEstimateReply(estimates, resolvedIntent.target.label),
        envelope.reply,
        demoData?.products ?? [],
      ),
    };
