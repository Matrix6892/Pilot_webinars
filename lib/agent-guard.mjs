...
function groundedBusinessContext({
  ...
  const quantityStep =
    product && !facts.requestedKg && !facts.surface
      ? "Письмо связано с каталогом. После ответа об объёме агент сверит живой склад и соберёт расчёт."
      : "";
  const industryLine =
    industry && !hypothesis
  ...
  if (/масл/iu.test(requestText)) {
    return [
      ...
      },
      {
        ...
        reply:
          "Технолог подберёт покрытие под контакт с моторным маслом. Пришлите название масла и опишите, как быстро его удаляют с пола. Остальные условия уже сохранены в карточке.",
      },
    ];
  }
  ...
  return [
    {
      ...
      rationale:
        "Технолог сопоставит название документа или средства с паспортом качества и карточкой краски.",
...
function fallbackReply(result, job, demoData, facts) {
    ...
    if (researchedFloor) {
      ...
      return {
        ...
        body: [
          ...
          `Если его используют как медицинский склад, пол бетонный, его моют нейтральным средством и по нему ездят тележки, первым вариантом станет краска «${sourceProduct.name}». Карточка товара подтверждает такую уборку, нагрузку от техники и паспорт качества на партию.`,
          colorLine,
          documentRequest,
  ...
  if (result.product) {
    ...
    const floorFitLine =
      ...
      facts.floorUse?.load &&
      sourceProduct
        ? `Для вашей задачи выбрали эту краску по карточке товара: она ${sourceProduct.features.join(
            ", ",
          )}.`
        : "";
