...
export function compoundReplyCoverageGaps(job, result) {
  ...
  const replyRoots = new Set(
    lexicalRoots(`${text(result?.reply?.subject)} ${text(result?.reply?.body)}`),
  );
  return requestParts
    .filter((part) => {
      const distinguishingRoots = lexicalRoots(part).filter(
        (root) => !compoundCoverageStopRoots.has(root),
      );
      return (
        distinguishingRoots.length > 0 &&
        !distinguishingRoots.some((root) => replyRoots.has(root))
      );
    })
    .map((part) => {
      const partRoots = new Set(
        lexicalRoots(part).filter(
          (root) => !compoundCoverageStopRoots.has(root),
        ),
      );
      return (
        goalParts.find((goalPart) =>
          lexicalRoots(goalPart).some(
            (root) =>
              !compoundCoverageStopRoots.has(root) && partRoots.has(root),
          ),
        ) ?? part
      );
    })
