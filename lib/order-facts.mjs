...
export function asksCompatibility(value) {
  ...
  for (let index = segments.length - 1; index >= 0; index -= 1) {
    if (rulesOutOilContact(segments[index])) return false;
...
export function hasSpecialTerms(value) {
  ...
  const namedCondition =
    /(скидк|тендер|конкурс\p{L}*\s+поставщик\p{L}*|штраф|не выше\s*\d|завтра|24\s*час|по образцу|специальн.*цвет|сертификат.*до)/iu.test(
      source,
    );
